...
export const FormBuilder: (props: IFormProps) => JSX.Element = ({
  ...
  const initialInputValue: IInputData = useMemo(
    () => ({ touched: false, value: undefined, error: '', required: false }),
    [],
  );
  ...
  useEffect(() => {
    setFormData(
      config.reduce(
        (initialFormData, { name, required }: IInputConfig) => ({
          ...initialFormData,
          [name]: { ...initialInputValue, required },
        }),
        {},
      ),
    );
  ...
  const formatFormData: () => Record<string, unknown> = () =>
    Object.keys(formData).reduce(
      (data: Record<string, unknown>, key: string) => ({
        ...data,
        [key]: _.get(formData, `[${key}.value]`, undefined),
      }),
      {},
    );
  ...
  const onInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData({
      ...
      [name]: {
        ...
        touched: true,
        error: await yup
          .reach(schemaGenerator(formatFormData()), name)
          .validate(value)
          .then(() => '')
  ...
  const renderInputs = () =>
    config.map(({ name, label, type, inputType, text, autocomplete }: IInputConfig) => (
      <Form.Group key={name}>
        ...
        {inputType === 'input' && (
          <Form.Control name={name} type={type} autoComplete={autocomplete} onChange={onInputChange} />
        )}
