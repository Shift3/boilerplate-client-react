...
export interface IApiService {
  ...
  post: <T, U>(endpoint: string, payload: U, config?: ApiRequestConfig) => Promise<T>;
  put: <T, U>(endpoint: string, payload: U, config?: ApiRequestConfig) => Promise<T>;
...
export class ApiService implements IApiService {
  ...
  private onResponseRejected = (error: AxiosError<ApiErrorResponse>) => {
    ...
    const errorResponse: ApiErrorResponse = { message: 'Unable to make request' };
    return Promise.reject(errorResponse);
  };
  ...
  public async post<T, U>(endpoint: string, payload: U, config?: ApiRequestConfig): Promise<T> {
    const requestConfig: AxiosRequestConfig = { ...config };
    const response = await this.axios.post<T>(endpoint, payload, requestConfig);
    return response.data;
  }

  public async put<T, U>(endpoint: string, payload: U, config?: ApiRequestConfig): Promise<T> {
    const requestConfig: AxiosRequestConfig = { ...config };
