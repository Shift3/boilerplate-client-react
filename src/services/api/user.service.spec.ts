...
import { IForgotPasswordRequest, IResetPasswordRequest, UserService } from './user.service';
  ...
  describe('resetPassword()', () => {
    ...
    describe('on success', () => {
      it('should return the requested user', async () => {
        ...
        const actual = await userService.resetPassword(resetPasswordRequest, token);
        expect(actual).toEqual(expected);
      });
    });
    ...
    describe('on error', () => {
      it('should reject with status message', async () => {
        ...
        expect(userService.resetPassword(resetPasswordRequest, token)).rejects.toEqual(expected);
      });
    });
  });
});
